...
Begin VB.Form modChien 
   BackColor       =   &H00AAD69B&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Modification d'un chien"
   ClientHeight    =   6720
   ClientLeft      =   3750
   ClientTop       =   1260
   ClientWidth     =   4575
   Icon            =   "modChien.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Image btNvRaceUp 
      Height          =   375
      Left            =   0
      Picture         =   "modChien.frx":824E
      Top             =   4800
      Visible         =   0   'False
      Width           =   2895
   End
End
Attribute VB_Name = "modChien"
...
Private Sub btFermerChien_Click()
    If saveChien() Then
        ...
        fGbl.datPrimaryRSChi.Refresh
        If fGbl.FrameConcours.Visible = True Then
            fGbl.btFermerConc_Click
        Else
...
Private Function saveChien() As Boolean
    ...
    Else
        ...
        'sauve
        datPrimaryRSChien.Recordset.UpdateBatch adAffectCurrent
        datPrimaryRSChien.Refresh
        datPrimaryRSChien.Refresh
...
'on sait pas sur quel bt il a cliqué
        'Récupération du chien
        datPrimaryRSChien.RecordSource = "select MaitreID,id,Nom,Race,Sexe,LOF,Tatouage,DateNaiss,no_CNEA,no_CUN from Chiens where id=" + id
        datPrimaryRSChien.Refresh
    If sex = 1 Then
        drpSex.ListIndex = 0
    Else
    ...
    End If
    datPrimaryRSRace.Recordset.MoveFirst
    While (idRaceChien <> idRace)
        datPrimaryRSRace.Recordset.MoveNext
    Wend
    editLabelRace = labelRace
